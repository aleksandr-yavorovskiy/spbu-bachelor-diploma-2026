...
\title[Настройка HW СХД]{\my@title@title@ru}
...
\begin{frame}{Введение}
    \begin{itemize}
        \item TATLIN.UNIFIED --- линейка универсальных CХД с двунодной конфигурацией КХ, разрабатываемая компанией YADRO
        \item BMC --- встроенный компонент, отвечающий за удаленное управление СХД
        \item На данный момент компанией YADRO разрабатывается новая аппаратная платформа
        \item Необходимо обеспечить корректную работу новой системы путем настройки аппаратных компонентов
    \end{itemize}
\end{frame}

\begin{frame}{Постановка задачи}
    \textbf{Целью} работы является разработка новой функциональности сервисов по настройке компонентов КХ для поддержки новой аппаратной платформы.
    \vspace{1em}

    \textbf{Задачи:}
    \begin{itemize}
        \item Реализовать механизм поддержки синхронизации пользовательских аккаунтов на BMC с КХ
        \item Разработать систему обновления прошивок аппаратных составляющих
        \item Реализовать систему синхронизации времени между аппаратными компонентами комплекса
        % \item Разработать подсистему взаимодействия с BMC в сервисе по управлению аппаратными компонентами СХД, реализующую:
        % \begin{itemize}
        %     \item создание и удаление пользовательских аккаунтов BMC;
        %     \item создание и удаление syslog подписок;
        %     \item установку механизма синхронизации времени в BMC;
        %     \item настройку LAN в BMC
        % \end{itemize}
        % \item Разработать подсистему конфигурирования BMC в сервисе по настройке аппаратуры при старте КХ
        \item Обеспечить тестовое покрытие новой функциональности:
        \begin{itemize}
            \item покрытие модульными тестами не ниже 75\%
            \item не менее 5 e2e тестов на каждую разработанную систему
        \end{itemize}
    \end{itemize}
\end{frame}
...
% Это всё пока закомментил для Представления темы
% \begin{frame}
